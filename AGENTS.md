...
- Every existing bordered control, card, tag, and divider should use the July 24 interaction: whenever a fine pointer is anywhere inside an element, its full existing edge reveals a restrained warm-white, blue, violet, and magenta glow, with a brighter hotspot oriented toward the pointer. Do not require border proximity. Keep the base border visible, activate only the nearest bordered element to avoid nested visual noise, disable the reactive glow for coarse pointers and reduced motion, and do not add extra decorative borders.
- Global fine-pointer movement should use the July 24 12:39 reference interaction: a broad, soft purple-indigo fluid ribbon with a bright inner filament, organic eddies, and a natural fade. It must remain click-through, preserve the independent inside-hover border glow, and be disabled for coarse pointers and reduced motion.
...
- Section-to-section motion should feel cinematic but restrained: use a one-time fine light sweep, whole-line masked title reveals, and short staggered image/content entrances. Keep the settled layout unchanged, avoid per-character effects or persistent decorative motion, use only transform/opacity/clip-path, reduce movement on small screens, and render sections statically for `prefers-reduced-motion`.
